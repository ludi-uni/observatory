...
The MCP server is a thin adapter. It does not search or scrape directly; it calls `OBSERVATION_SERVICE_URL`. It supports stdio for IDE compatibility and Streamable HTTP for long-running DOLL / OCI deployments.
...
For the resident HTTP server, use `http://127.0.0.1:52036/mcp` locally or the internal Compose URL in OCI. `/health` is unauthenticated for container health checks; `/mcp` uses the same Bearer key as the observation service when `OBSERVATION_API_KEY` is set.
